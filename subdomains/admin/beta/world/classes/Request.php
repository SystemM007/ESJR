...
class Request extends RequestBasic
{
	...
	private static function startSection($ID, $siteModule, $paramAllowed)
	{
		...
		/*
		 * Dit is de meest basale vorm van het gebruiken van de cache.
		 * Het scheelt geen queries / rekenkracht, enkel bandbreedte en de tijd van het overdragen van de data
		 * 
		 * Verdere uitbreiding zou zijn dat de "eenvoudige" pagina's een "Last Mod" checken VOORDAT ze gaan laden / templates gaan lezen enzovoorts.
		 */
		...
		$Cache->sendCacheControl(HTTPCache::CACHE_PRIVATE);
		$Cache->useEtag(hash("md5", $buffer));
