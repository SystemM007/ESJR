...
class MySql extends MySqlBasic{
	...
	public static function escape($str)
	{
		if(self::$disableEscape){
			...
			return self::$dbc->real_escape_string($str);
		}
	}
	...
	private static function decodeSelect($sq){
		...
		if(isset($join))
		{
			...
			foreach($join as $singleJoin)
			{
				...
				if(isset($singleJoin["type"]) && $singleJoin["type"] == "right") $q .= " \nRIGHT JOIN"; 
				else $q .= " \nLEFT JOIN ";
				
				
				if(!isset($singleJoin["table"])) throw new Exception("Geen table gevonden in JOIN");
				$q .= "" . $singleJoin["table"] . " ";
				
				if(isset($singleJoin["using"])) $q .= " USING (" . $singleJoin["using"] . ")";
				elseif(isset($singleJoin["on"])) $q .= " ON " . $singleJoin["on"] . "";
	...
	{
		if(!isset($sq["select"]) || !$sq["select"]) $sq["select"] = "*";
