...
abstract class RequestBasic
{
	...
	{	
		...
		// waarom had ik hier REDIRECT_URL gebruikt?!
		self::$originalPath = parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH);
...
//		register_shutdown_function(array("RequestBasic", "uploadCleanUp"));
	...
	protected static function readJsonPostField($field)
	{
		...
		if(isset($_POST[$field]))
		{
			...
			if(!is_array($r)) trigger_error("Op \$_POST[$field] werd een JSON string verwacht, deze is onjuist geformatteerd of leeg: ". $json ,e);
	...
	{
		return ( self::$Type & self::TYPE_AJAX == self::TYPE_AJAX);
	}
	...
	public static function give404($msg = "")
	{
		...
		echo new Fragment_ErrorPage(404, $msg);
		exit;
	}
	...
	public static function give403($msg = "")
	{
		...
		echo new Fragment_ErrorPage(403, $msg);
		exit;
	}
